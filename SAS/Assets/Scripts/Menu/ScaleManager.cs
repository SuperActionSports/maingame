...
using InControl;
	...
	public class ScaleManager : MonoBehaviour
	{
		...
		List<Vector3> scalePositions = new List<Vector3>() {
			new Vector3( -1, 1, -10 ),
			new Vector3( 1, 1, -10 ),
			new Vector3( -1, -1, -10 ),
			new Vector3( 1, -1, -10 ),
		...
		Scale CreateScale( InputDevice inputDevice )
		{
			if (scales.Count < maxScales)
			{
				...
				var playerPosition = scalePositions[0];
				scalePositions.RemoveAt( 0 );
		...
		{
			scalePositions.Insert( 0, player.transform.position );
