...
public class PlayerController3D : MonoBehaviour {
    ...
    {
		if (device == null )
			return GetKeyboardXInput();
		else 
			return GetControllerXInput();
    }
	
	/*private float GetYVelocity()
	{
		if (device == null )
			return GetKeyboardYInput();
		else 
			return GetControllerYInput();
	}*/

	private float GetZVelocity()
	{
		if (device == null )
			return GetKeyboardZInput();
		else 
			return GetControllerZInput();
	}
	/*
	...
	/*private float GetKeyboardYInput()
	{
		if (Input.GetKey(up))
		{
			magSpeedY = -1;
		...
		return speedMagnitude * magSpeedY * Time.deltaTime;
	}
	
	private float GetControllerYInput()
	{
